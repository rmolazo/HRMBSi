...
if(!isset($_GET['email'])){
    echo'<script>location.replace("recovery.php");</script>';
}else{
...
if(isset($_POST['submit'])){
   ...
   $select_user = $conn->prepare("SELECT * FROM `otp` WHERE email = ? AND pin = ? ORDER BY id DESC LIMIT 1");
   $select_user->execute([$user_email, $pin]);
   $row = $select_user->fetch(PDO::FETCH_ASSOC);
   
   if($select_user->rowCount() > 0){
     $delete_otp = $conn->prepare("DELETE FROM `otp` WHERE email = ?");
     $delete_otp->execute([$user_email]);
     echo "<script>window.open('changepassword.php?email=" . $user_email . "', '_self')</script>";
      
   }else{
      $message[] = 'Pin not recognized!';
   }
...
if(isset($_POST['resend'])){
   ...
   if($select_user->rowCount() > 0){
     ...
     $msg = "Hello! $user_email, your pin is $pin.";
     $insert_otp = $conn->prepare("INSERT INTO `otp` (`email`, `pin`,`date`) VALUES (?,?,?)");
     $insert_otp->execute([$user_email, $pin, $timeactivity]);
     ...
     $phpmailer->send();
     echo "<script>window.open('pin.php?email=" . $user_email . "', '_self')</script>";
   }else{
...
<section class="form-container">
   ...
   <form action="" method="post" enctype="multipart/form-data" class="login">
      ...
      <center><span style="font-size: 15px;"><b>NOTE:</b> Please check your email for a message with your code. Your code is 6 numbers long.</span></center>
      <br>
      <center><span style="font-size: 15px;">We sent your code to <?php echo $user_email ?></span></center>
      <p>Pin <span>*</span></p>
      <input type="password" name="pin" id ="pass" placeholder="Please enter your one time pin" minlength ="6 "maxlength="6" autocomplete class="box">
      Show Pin <i class="bi bi-eye-slash" id="togglePassword" style="font-size: 15px;"></i>
      <p class="link">Login your account? <a href="login.php">Login</a></p>
      <center><input type="submit" name="submit" value="Submit" class="inline-btn">
      <input type="submit" name="resend" value="Resend Code?" class="inline-btn"></center>
   </form>
