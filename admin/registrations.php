...
if(isset($_POST['delete'])){
   ...
   $verify_playlist = $conn->prepare("SELECT * FROM `registration` WHERE id = ? LIMIT 1");
   $verify_playlist->execute([$delete_id]);

   if($verify_playlist->rowCount() > 0){

   $delete_user = $conn->prepare("DELETE FROM `registration` WHERE id = ?");
   $delete_user->execute([$delete_id]);
   $message[] = 'User deleted!';
   }else{
      $message[] = 'Information already deleted!';  
   }
}
...
<section class="playlists">
   ...
   <div class="box-container">
      ...
      <?php
        ...
        while ($rowreg = $users->fetch()) {
        $id = $rowreg['id'];
        $email = $rowreg['email'];
        $event_title= $rowreg['event_title'];  
        $attendees= $rowreg['attendees'];   
      ?>
      ...
      <div class="box">
        <!--<div><i class="fas fa-circle-dot" style="<?php if($rowuser['status'] == 'Active'){echo 'color:limegreen'; }else{echo 'color:red';} ?>"></i> <span style="<?php if($rowuser['status'] == 'Active'){echo 'color:limegreen'; }else{echo 'color:red';} ?>"><?= $rowuser['status']; ?></span></div>-->
         <h3 class="title"><b>Title:</b> <?= $rowreg['event_title']; ?></h3>
         <p class="title"><b>Email:</b> <?= $rowreg['email']; ?></p>
         <p class="title"><b>Attendees:</b> <?= $rowreg['attendees']; ?></p>
         <form action="" method="post" class="flex-btn">
            <input type="hidden" name="id" value="<?= $id; ?>">
            <!--<a href="update_user.php?get_id=<?= $id; ?>" class="option-btn"><i class="fas fa-pen"></i> Update</a>-->
            <input type="submit" value="delete" class="delete-btn" onclick="return confirm('Delete this information?');" name="delete">
         </form>
