...
$fetch_user = $select_user->fetch(PDO::FETCH_ASSOC);
             ...
             $select_contents->execute([$get_id]);
             ...
             $fetch_tutor = $select_tutor->fetch(PDO::FETCH_ASSOC);
                         ...
                         $email = $fetch_tutor['email'];
                         $image = $fetch_tutor['image'];
                         //$status = $fetch_tutor['status'];//
          
             $select_admin_activity = $conn->prepare("SELECT * FROM `loginactivity` WHERE user_id = ? ORDER BY time DESC");
             $select_admin_activity->execute([$email]);
             $total_admin_logins = $select_admin_activity->rowCount();
             if($total_admin_logins > 0){
               $fetch_admin_login = $select_admin_activity->fetch(PDO::FETCH_ASSOC);
               $last_admin_login = $fetch_admin_login['time'];
               } else {
               $last_admin_login = 'No logins yet';
               }
?>
...
<head>
   ...
   <title>HRMBSi | Admin Profile</title>
   ...
   <div class="details">
      ...
      <div class="box">
            <div class="flex">
               ...
               <div>
                  <span>Logins: <?php echo $total_admin_logins . ', ' . 'Last Login: ' . $last_admin_login; ?></span>
               </div>
            </div>
         </div>
      </div>
